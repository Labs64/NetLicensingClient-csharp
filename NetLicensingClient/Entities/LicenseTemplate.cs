...
namespace NetLicensingClient.Entities
{
    ...
    /// https://go.netlicensing.io/javadoc/v2/com/labs64/netlicensing/domain/entity/LicenseTemplate.html
    /// </summary>
    public class LicenseTemplate : BaseEntity
    {
        ...
        internal LicenseTemplate(item source)
        {
            ...
            {
                throw new NetLicensingException(String.Format("Wrong object type '{0}', expected '{1}'", (source.type != null) ? source.type : "<null>", Constants.LicenseTemplate.TYPE_NAME));
            }
            licenseTemplateProperties = new Dictionary<String, String>();
            foreach (property p in source.property)
            {
                switch (p.name)
                {
                    ...
                    default:
                        ...
                        {
                            // custom property
                            if (!licenseTemplateProperties.ContainsKey(p.name))
                            {
        ...
        internal new Dictionary<String, String> ToDictionary()
        {
            ...
            if (price.HasValue) dict[Constants.PRICE] = price.Value.ToString("F2", Utilities.NetLicensingNumberFormat);
